 ...
 @ViewChild('printSection') printSection!: ElementRef;
  ...
  onSubmit(): void {
    if (this.addUserForm.valid) {
      const userData = {username :this.addUserForm.value.username, password:this.addUserForm.value.password};
      
      // Appel du service pour ajouter un utilisateur
      this.authService.addUser(userData,this.addUserForm.value.role).subscribe({
        next: () => {
